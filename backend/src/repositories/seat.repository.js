...
exports.createSeat = async (seatData) => {
	try {
		...
		const res = await db.query(
			"INSERT INTO seats (event_id, position, hierarchy, price) VALUES ($1, $2, $3, $4) RETURNING *",
			[event_id, position, hierarchy, price],
		);
...
exports.updateSeat = async (seatId, seatData) => {
	try {
		const { event_id, position, hierarchy, price } = seatData;
		const res = await db.query(
			"UPDATE seats SET event_id = $1, position = $2, hierarchy = $3, price = $4 WHERE id = $5 RETURNING *",
			[event_id, position, hierarchy, price, seatId],
		);
		if (res.rows.length === 0) {
...
exports.deleteSeat = async (seatId) => {
	try {
		const res = await db.query("DELETE FROM seats WHERE id = $1 RETURNING *", [
			seatId,
		]);
		if (res.rows.length === 0) {
