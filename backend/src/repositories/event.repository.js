...
exports.getEventByLocationAndDate = async (locationId, date) => {
	try {
		const res = await db.query(
			"SELECT * FROM events WHERE location_id = $1 AND DATE(date) = DATE($2)",
			[locationId, date],
		);
		return res.rows;
	} catch (error) {
...
exports.getEventByDate = async (date) => {
	try {
		const res = await db.query(
			"SELECT * FROM events WHERE DATE(date) = DATE($1) ORDER BY date",
			[date],
		);
		return res.rows;
	} catch (error) {
...
exports.getEventByName = async (name) => {
	try {
		const res = await db.query("SELECT * FROM events WHERE name ILIKE $1", [
			`%${name}%`,
		]);
		return res.rows;
	} catch (error) {
