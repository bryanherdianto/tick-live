...
exports.createTicket = async (ticketData) => {
	try {
		...
		const res = await db.query(
			"INSERT INTO tickets (seat_id, user_id) VALUES ($1, $2) RETURNING *",
			[seat_id, user_id],
		);
...
exports.updateTicket = async (ticketId, ticketData) => {
	try {
		...
		const res = await db.query(
			"UPDATE tickets SET user_id = $1, seat_id = $2, status = $3 WHERE id = $4 RETURNING *",
			[user_id, seat_id, status, ticketId],
		);
		if (res.rows.length === 0) {
